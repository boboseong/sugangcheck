(function() {
	function W({ records: e }, u) {
		return e.flatMap((p) => u.some((t) => t.target.grade === p.target.grade && t.target.semester === p.target.semester && t.normalizedSubjectName === p.normalizedSubjectName) ? [] : [n({
			ruleId: "courseExistsInOperatingSubjects",
			studentId: p.studentId,
			studentNo: p.studentNo,
			studentName: p.studentName,
			message: `${p.subjectName} 과목이 해당 학기 운영과목에 없습니다.`,
			relatedRecordIds: [p.id]
		})]);
	}
	function i(e) {
		const u = /* @__PURE__ */ new Map();
		for (const p of e) {
			const t = u.get(p.studentId);
			t ? t.push(p) : u.set(p.studentId, [p]);
		}
		return u;
	}
	function G(e) {
		return e.reduce((u, p) => u + p.credits, 0);
	}
	function l(e) {
		return e[0];
	}
	...
	[...P];
	function I(e) {
		return `${e.grade}-${e.semester}`;
	}
	function d(e) {
		return `${e.grade}학년 ${e.semester}학기`;
	}
	function Z(e) {
		const u = /^([1-3])-([1-2])$/.exec(e.trim());
		if (u) return {
			grade: Number(u[1]),
			semester: Number(u[2])
		};
	...
	function J(e, u) {
		return e.grade - u.grade || e.semester - u.semester;
	}
	function X(e, u) {
		return [...e].sort((p, t) => J(u(p), u(t)));
	}
	const Y = /[·ㆍ•･∙‧・⋅]/g, Q = /[-‐‑‒–—―]/g, ee = /[()[\]{}<>〈〉《》「」『』【】〔〕]/g, ue = /[\\/]/g, pe = /[_+|]/g, te = {
		IV: "4",
		III: "3",
		II: "2",
		I: "1"
	...
	function _(e) {
		return te[e.toUpperCase()] ?? e;
	}
	function se(e) {
		let u = e.replace(/(^|[^A-Za-z가-힣0-9])(IV|III|II|I)(?=$|[^A-Za-z가-힣0-9])/gi, (p, t, s) => `${t}${_(s)}`);
		return u = u.replace(/([가-힣])(IV|III|II|I)(?=$|[^A-Za-z가-힣0-9])/g, (p, t, s) => `${t} ${_(s)}`), u;
	}
	function oe(e) {
		return e.replace(/([가-힣A-Za-z])([1-4])(?=$|[^0-9])/g, "$1 $2").replace(/(^|[^0-9])([1-4])([가-힣A-Za-z])/g, "$1$2 $3");
	}
	function ce(e) {
		return e.replace(Y, " ").replace(Q, " ").replace(ue, " ").replace(pe, " ").replace(ee, " ");
	}
	function C(e) {
		return e == null ? "" : oe(se(ce(String(e).normalize("NFKC")))).trim().replace(/\s+/g, " ").toLowerCase();
	}
	const re = [
		{
			subjectName: "공통국어1",
	...
	function S(e) {
		return [...new Set(e)].filter(Boolean).sort((u, p) => u.localeCompare(p, "ko"));
	}
	const A = re.map((e, u) => ({
		id: `subject-master-${u + 1}`,
		subjectName: e.subjectName,
		normalizedSubjectName: C(e.subjectName),
		subjectGroup: e.subjectGroup,
		selectionType: e.selectionType,
		groupType: e.groupType
	}));
	S(A.map((e) => e.subjectGroup));
	S(A.map((e) => e.selectionType));
	S(A.map((e) => e.groupType ?? ""));
	const v = "semesterCreditSubjectCriteria";
	function M(e) {
		const u = typeof e == "number" ? e : Number(String(e).trim());
		return Number.isFinite(u) && u >= 0 ? u : void 0;
	}
	function be(e) {
		return [...new Set(e)].sort((u, p) => u - p);
	}
	function R(e) {
		if (Array.isArray(e)) {
			const p = e.map(M).filter((t) => t !== void 0);
			return p.length > 0 ? be(p) : [0];
		}
		const u = M(e);
		return u === void 0 ? [0] : [u];
	}
	function w(e) {
		return R(e).filter((u) => u > 0);
	}
	function je(e) {
		if (typeof e != "object" || e === null || !("target" in e)) return;
		const u = e.target, p = Number(u?.grade), t = Number(u?.semester);
		if ((p === 1 || p === 2 || p === 3) && (t === 1 || t === 2)) return {
			grade: p,
			semester: t
		};
	}
	function ye(e, u) {
		...
		return {
			target: e,
			allowedCredits: R(p.allowedCredits),
			allowedSubjectCounts: R(p.allowedSubjectCounts)
		};
	}
	function Te(e) {
		const u = e[v], p = Array.isArray(u) ? u : [], t = /* @__PURE__ */ new Map();
		for (const s of p) {
			const o = je(s);
			o && t.set(I(o), s);
		}
		return P.map((s) => ye(Z(s), t.get(s)));
	}
	function ne(e) {
		return new Map(e.map((u) => [I(u.target), u]));
	}
	function L(e) {
		return `${e.target.grade}-${e.target.semester}`;
	}
	function z(e) {
		const u = /* @__PURE__ */ new Map();
		for (const p of e) u.set(p, (u.get(p) ?? 0) + 1);
		return [...u.entries()].sort((p, t) => t[1] - p[1] || p[0] - t[0])[0]?.[0];
	}
	function D(e) {
		return new Set(e.map((u) => u.normalizedSubjectName)).size;
	}
	function q(e, u) {
		return e.map((p) => `${p}${u}`).join(", ");
	}
	function ae({ records: e, setting: u }) {
		const p = typeof u.criteria.maxDifferenceCredits == "number" ? u.criteria.maxDifferenceCredits : 0, t = Array.isArray(u.criteria[v]), s = ne(Te(u.criteria)), o = /* @__PURE__ */ new Map(), c = [];
		for (const b of e) {
			const j = L(b);
			o.set(j, [...o.get(j) ?? [], b]);
		}
		for (const b of o.values()) {
			const j = [...i(b).values()], r = j.map(G), T = j.map(D), a = z(r), K = z(T);
			if (a === void 0) continue;
			const B = l(b), F = B ? s.get(L(B)) : void 0, O = w(F?.allowedCredits ?? []), H = w(F?.allowedSubjectCounts ?? []), h = O.length > 0 ? O : void 0, $ = H.length > 0 ? H : t && K !== void 0 ? [K] : void 0;
			for (const g of j) {
				const f = G(g), U = D(g), y = l(g);
				y && (h ? h.includes(f) || c.push(n({
					ruleId: "creditDifference",
					instanceId: "credits",
					studentId: y.studentId,
					studentNo: y.studentNo,
					studentName: y.studentName,
					message: `${d(y.target)} 이수학점 ${f}학점이 허용값 ${q(h, "학점")}과 맞지 않습니다.`,
					relatedRecordIds: g.map((N) => N.id),
					semester: y.target
				})) : Math.abs(f - a) > p && c.push(n({
					ruleId: "creditDifference",
					studentId: y.studentId,
					studentNo: y.studentNo,
					studentName: y.studentName,
					message: `${d(y.target)} 최빈 이수학점 ${a}학점과 ${Math.abs(f - a)}학점 차이가 납니다.`,
					relatedRecordIds: g.map((N) => N.id),
					semester: y.target
				})), $ && !$.includes(U) && c.push(n({
					ruleId: "creditDifference",
					...
					studentNo: y.studentNo,
					studentName: y.studentName,
					message: `${d(y.target)} 과목 수 ${U}개가 허용값 ${q($, "개")}와 맞지 않습니다.`,
					relatedRecordIds: g.map((N) => N.id),
					semester: y.target
				})));
			}
	...
	function E(e) {
		return `${d(e.target)} ${e.subjectName}`;
	}
	function V(e, u) {
		return e.target.grade === u.target.grade && e.target.semester === u.target.semester && e.normalizedSubjectName === u.normalizedSubjectName;
	}
	function x(e, u) {
		return u.includeExternalInputsOverride ?? !0 ? [...e] : e.filter((p) => p.origin.type === "courseSelectionFile");
	}
	function ie(e) {
		return `${I(e.target)}|${e.normalizedSubjectName}`;
	}
	function le(e) {
		const u = /* @__PURE__ */ new Map();
		return e.subjects.forEach((p) => {
			const t = e.records.find((s) => V(s, p));
			t && u.set(ie(p), t);
		}), [...u.values()];
	}
	function k(e) {
		return {
			...
			relatedSubjectNames: e.relatedSubjectNames
		};
	}
	function me(e) {
		const u = x(e.studentRecords, e.rule), p = u.filter((s) => V(s, e.rule.trigger));
		if (p.length === 0) return [];
		if (u.some((s) => V(s, e.rule.required))) return [];
		const t = p[0];
		return t ? [k({
			rule: e.rule,
			student: t,
			message: `${E(e.rule.trigger)}를 이수했지만 ${E(e.rule.required)}를 이수하지 않았습니다.`,
			relatedRecordIds: p.map((s) => s.id),
			relatedSubjectNames: [e.rule.trigger.subjectName, e.rule.required.subjectName]
		})] : [];
	}
	function ge(e) {
		const u = le({
			records: x(e.studentRecords, e.rule),
			subjects: e.rule.subjects
		}), p = u.length, t = e.rule.comparison === "atLeast" ? p >= e.rule.count : p <= e.rule.count, s = l(e.studentRecords);
		if (!t || !s) return [];
		const o = e.rule.comparison === "atLeast" ? "이상" : "이하";
		return [k({
			rule: e.rule,
			student: s,
			message: `${e.rule.name} 조건의 지정 과목을 ${p}개 선택하여 기준 ${e.rule.count}개 ${o}에 해당합니다.`,
			relatedRecordIds: u.map((c) => c.id),
			relatedSubjectNames: e.rule.subjects.map((c) => c.subjectName)
		})];
	}
	function Ne({ records: e }, u) {
		const p = u.filter((s) => s.status === "active"), t = [];
		for (const s of i(e).values()) for (const o of p) t.push(...o.type === "linkedSubject" ? me({
			rule: o,
			studentRecords: s
		}) : ge({
			rule: o,
	...
	function Ge(e) {
		return [...new Set(X(e, (u) => u.target).map((u) => I(u.target)))].join(", ");
	}
	function Ie({ records: e }) {
		const u = [];
		for (const p of i(e).values()) {
			const t = /* @__PURE__ */ new Map();
			for (const s of p) t.set(s.normalizedSubjectName, [...t.get(s.normalizedSubjectName) ?? [], s]);
			for (const s of t.values()) {
				...
				o && u.push(n({
					...
					message: `${o.subjectName} 과목이 ${Ge(s)}에 중복 신청되었습니다.`,
					relatedRecordIds: s.map((c) => c.id)
				}));
			}
		}
		return u;
	}
	function de({ records: e, setting: u }) {
		const p = typeof u.criteria.requiredCredits == "number" ? u.criteria.requiredCredits : 6, t = Array.isArray(u.criteria.normalizedSubjectNames) ? u.criteria.normalizedSubjectNames : ["한국사1", "한국사2"].map(C), s = new Set(t), o = [];
		for (const c of i(e).values()) {
			const b = l(c), j = c.filter((T) => s.has(T.normalizedSubjectName)), r = j.reduce((T, a) => T + a.credits, 0);
			b && r < p && o.push(n({
				ruleId: "koreanHistoryCredits",
				studentId: b.studentId,
				studentNo: b.studentNo,
				studentName: b.studentName,
				message: `한국사 이수학점 ${r}학점이 기준 ${p}학점보다 적습니다.`,
				relatedRecordIds: j.map((T) => T.id)
			}));
		}
		return o;
	}
	Array.from({ length: 27 }, (e, u) => 174 + u);
	["한국사1", "한국사2"].map(C);
	const m = {
		subjectGroups: [
			"국어",
		...
		excessCreditRatioLimit: .5,
		note: "총 교과 이수학점 174학점까지는 국어·수학·영어 합산 81학점 초과 여부를 보고, 174학점을 초과하면 초과 이수학점의 50%를 넘지 않도록 계산한다."
	};
	function fe(e) {
		return {
			subjectGroups: Array.isArray(e?.subjectGroups) ? e.subjectGroups : m.subjectGroups,
			baselineTotalCredits: typeof e?.baselineTotalCredits == "number" ? e.baselineTotalCredits : m.baselineTotalCredits,
			baselineLimitCredits: typeof e?.baselineLimitCredits == "number" ? e.baselineLimitCredits : m.baselineLimitCredits,
			excessCreditRatioLimit: typeof e?.excessCreditRatioLimit == "number" ? e.excessCreditRatioLimit : m.excessCreditRatioLimit,
			note: typeof e?.note == "string" ? e.note : m.note
		};
	}
	function Ce(e, u = m) {
		return u.baselineLimitCredits + Math.max(0, e - u.baselineTotalCredits) * u.excessCreditRatioLimit;
	}
	function Se({ records: e, setting: u }) {
		const p = fe(u.criteria), t = new Set(p.subjectGroups), s = [];
		for (const o of i(e).values()) {
			const c = Ce(G(o), p), b = o.filter((r) => t.has(r.subjectGroup)).reduce((r, T) => r + T.credits, 0), j = l(o);
			j && b > c && s.push(n({
				ruleId: "koreanMathEnglishLimit",
				studentId: j.studentId,
				studentNo: j.studentNo,
				studentName: j.studentName,
				message: `국어·수학·영어 합산 ${b}학점이 제한 ${c}학점을 초과합니다.`,
				relatedRecordIds: o.filter((r) => t.has(r.subjectGroup)).map((r) => r.id)
			}));
		}
		return s;
	}
	function Ae({ records: e, setting: u }) {
		const p = typeof u.criteria.minimumTotalCredits == "number" ? u.criteria.minimumTotalCredits : 174, t = [];
		for (const s of i(e).values()) {
			const o = G(s), c = l(s);
			c && o < p && t.push(n({
				...
				message: `총 이수학점 ${o}학점이 최소 기준 ${p}학점보다 적습니다.`,
				relatedRecordIds: s.map((b) => b.id),
	...
	function Re(e, u) {
		return e.target.grade - u.target.grade || e.target.semester - u.target.semester;
	}
	function Ve({ records: e }, u) {
		const p = u.filter((s) => s.status === "active"), t = [];
		for (const s of i(e).values()) for (const o of p) {
			const c = o.includeExternalInputsOverride === !1 ? s.filter((r) => r.origin.type === "courseSelectionFile") : s, b = c.filter((r) => r.normalizedSubjectName === o.beforeNormalizedSubjectName), j = c.filter((r) => r.normalizedSubjectName === o.afterNormalizedSubjectName);
			for (const r of j) b.some((T) => {
				const a = Re(T, r);
				return o.allowConcurrent ? a <= 0 : a < 0;
			}) || t.push(n({
				...
				message: `${r.subjectName} 이수 전 ${o.beforeSubjectName} 이수가 필요합니다.`,
				relatedRecordIds: [r.id, ...b.map((T) => T.id)]
			}));
		}
		return t;
	}
	function he(e) {
		const u = e.requiredSubjectGroupCredits;
		return Array.isArray(u) ? u : [];
	}
	function $e({ records: e, setting: u }) {
		const p = [];
		for (const t of i(e).values()) {
			const s = l(t);
			if (s) for (const o of he(u.criteria)) {
				const c = /* @__PURE__ */ new Set([o.subjectGroup, ...o.equivalentSubjectGroups ?? []]), b = t.filter((j) => c.has(j.subjectGroup)).reduce((j, r) => j + r.credits, 0);
				b < o.requiredCredits && p.push(n({
					ruleId: "requiredSubjectGroupCredits",
					instanceId: o.subjectGroup,
					studentId: s.studentId,
					studentNo: s.studentNo,
					studentName: s.studentName,
					message: `${o.subjectGroup} 이수학점 ${b}학점이 기준 ${o.requiredCredits}학점보다 적습니다.`,
					relatedRecordIds: t.map((j) => j.id)
				}));
			}
		}
		return p;
	}
	function Pe({ records: e }, u) {
		const p = [];
		for (const t of e) {
			const s = u.find((c) => c.target.grade === t.target.grade && c.target.semester === t.target.semester && c.normalizedSubjectName === t.normalizedSubjectName);
			if (!s) continue;
			const o = [
				s.subjectGroup !== t.subjectGroup ? "교과군" : void 0,
				s.selectionType !== t.selectionType ? "선택구분" : void 0,
				s.groupType && t.groupType && s.groupType !== t.groupType ? "과목구분" : void 0,
				s.credits !== t.credits ? "학점" : void 0
			].filter(Boolean);
			o.length > 0 && p.push(n({
				...
				message: `${t.subjectName} 과목 정보가 운영과목과 다릅니다: ${o.join(", ")}`,
				relatedRecordIds: [t.id]
			}));
		}
		return p;
	}
	function _e(e, u) {
		return u.id === "prerequisites" || u.id === "detailedConstraints" ? [...e] : u.includeExternalInputs ? [...e] : e.filter((p) => p.origin.type === "courseSelectionFile");
	}
	function n({ ruleId: e, instanceId: u, type: p = e, studentId: t, studentNo: s, studentName: o, message: c, relatedRecordIds: b, semester: j, fixHint: r }) {
		return {
			id: [
				e,
				u,
	...
	function ve(e, u) {
		const p = performance.now(), t = [], s = [], o = [];
		for (const c of e.ruleSettings) {
			const b = u[c.id];
			if (!c.enabled || !b) {
				o.push(c.id);
				continue;
			...
			const j = b({
				mode: e.mode,
				records: _e(e.records, c),
				setting: c
			});
			t.push(...j), s.push(c.id);
		}
		return {
			errors: t,
			executedRuleIds: s,
	...
	function Me(e) {
		const { detailedConstraintRules: u, operatingSubjects: p, prerequisiteRules: t, ...s } = e;
		return ve(s, we({
			detailedConstraintRules: u,
			operatingSubjects: p,
			prerequisiteRules: t
		}));
	}
	function we(e) {
		return {
			minimumCredits: Ae,
			creditDifference: ae,
			requiredSubjectGroupCredits: $e,
			koreanHistoryCredits: de,
			koreanMathEnglishLimit: Se,
			duplicateSubjects: Ie,
			prerequisites: (u) => Ve(u, e.prerequisiteRules),
			detailedConstraints: (u) => Ne(u, e.detailedConstraintRules),
			courseExistsInOperatingSubjects: (u) => W(u, e.operatingSubjects),
			subjectMetadataMismatch: (u) => Pe(u, e.operatingSubjects)
		};
	}
	self.addEventListener("message", (e) => {
		...
		const { requestId: u, payload: p } = e.data;
		try {
			const t = {
			...
			const s = {
				...
				requestId: u,
				message: t instanceof Error ? t.message : "점검 실행 중 오류가 발생했습니다."
			};
			self.postMessage(s);
		}
	});
